---
title: "the mem_write c memory primitive"
---
...
(just `memcpy` that uses `u64` instead of `size_t`) and it was kinda error prone one could say.
...
> NOTE: It's not possible to create a macro: `#define STR_RANGE(str) (str), ((str) + sizeof(str) / sizeof((str)[0]))`.
> It would expand a `str` literal twice which compilers do not guarantee that would point to the same location (string pooling).
> And in fact msvc (at least in `/Od` builds) creates multiple `str` literals in this case.
...
like `void* mem_zero(void* ptr, u64 size)` which is equivalent to `memset(ptr, 0, size)`.
...
> This is why I wrote `MEM` as `#define MEM(array) &(array), sizeof(array)` and not `#define MEM(array) (array), sizeof(array)`
...
A little caveat to keep in mind though: `MEM` will include the trailing `\0` when used with string literals .
